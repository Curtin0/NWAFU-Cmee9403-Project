...
class mywindow(QtWidgets.QWidget, Ui_MainWindow):
    ...
    def signalTrigger(self, event):
        if self.value_judgement == 0:
            if self.cap_front.isOpened() and self.cap_back.isOpened():
                ...
                GPIO.add_event_detect(40, GPIO.FALLING, callback = callback_back, bouncetime = 200)
    ...
    def closeEvent(self, event):
        ...
        yes.setText("确定")
        no.setText("取消")
        reply = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Warning, "提示", "退出后将停止装置运行,\n你确认要退出吗？")
        reply.addButton(yes,QtWidgets.QMessageBox.ActionRole)
        reply.addButton(no, QtWidgets.QMessageBox.RejectRole)
...
class mysubwindow(QtWidgets.QWidget, Ui_SubWindow):
    def __init__(self):
        super(mysubwindow,self).__init__()
        self.setupUi(self)
        
        self.button_returning.clicked.connect(self.returning)
    ...
    def returning(self, event):
        ...
        yes.setText("确定")
        no.setText("取消")
        reply = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Warning, "提示", "您确定开始运行装置吗?")
        reply.move(520,350)
        reply.addButton(yes, QtWidgets.QMessageBox.ActionRole)
        reply.addButton(no, QtWidgets.QMessageBox.RejectRole)
        ...
        if reply.exec_() == QtWidgets.QMessageBox.RejectRole:
            ...
            # pwm_feeding = GPIO.PWM(12, 400)
            # pwm_detecting = GPIO.PWM(12, 400)
            
            text= self.comboBox.currentText()
                
            if text == "一档速度":
                # pwm_feeding.start(60)
                    print("1")
                
            elif text == "二档速度":
                # pwm_feeding.start(80)
                print("2")
                
            else:
                # pwm_feeding.start(100)
                print("3")

if __name__ == "__main__":
    app = QtWidgets.QApplication(sys.argv)
    myshow = mywindow()
    mysubshow = mysubwindow()
    myshow.show()
    myshow.button_workingSpeed.clicked.connect(mysubshow.workingSpeed)
    sys.exit(app.exec_())